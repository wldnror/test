...
def set_angle(pwm, angle, calibration_offset):
    duty = 2 + ((angle + calibration_offset) / 18)
...
def main(stdscr):
    ...
    angle1 = 90
    angle2 = 90
    set_angle(pwm1, angle1, calibration_offset_1)
    set_angle(pwm2, angle2, calibration_offset_2)

    stdscr.addstr(0, 0, "서보 모터 제어 프로그램")
    stdscr.addstr(1, 0, "각도를 입력하고 Enter를 누르세요 (변경 각도 음수/양수)")
    ...
    while True:
        ...
        try:
            angle1 += int(angle1_change)
            angle2 += int(angle2_change)
            if 0 <= angle1 <= 180 and 0 <= angle2 <= 180:
                ...
                angle1 -= int(angle1_change)  # 범위를 벗어나면 원래 각도로 복원
                angle2 -= int(angle2_change)  # 범위를 벗어나면 원래 각도로 복원
        except ValueError:
